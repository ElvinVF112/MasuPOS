...
import type { CategProveedorOption } from "@/lib/pos-data"
import { useUnsavedGuard } from "@/lib/unsaved-guard"
import { DeleteConfirmModal } from "@/components/pos/delete-confirm-modal"

type ItemForm = { id?: number; code: string; name: string }
const emptyForm: ItemForm = { code: "", name: "" }

function recordToForm(r: CategProveedorOption): ItemForm {
  return { id: r.id, code: r.code, name: r.name }
}

function duplicateRecordToForm(r: CategProveedorOption): ItemForm {
  return { code: "", name: `${r.name} COPIA` }
}

export function CatalogSupplierCategoriesScreen({ initialData }: { initialData: CategProveedorOption[] }) {
  const router = useRouter()
  const { t } = useI18n()
  const { setDirty, confirmAction } = useUnsavedGuard()
  const menuRef = useRef<HTMLUListElement | null>(null)

  const [items, setItems] = useState<CategProveedorOption[]>(initialData)
  ...
  const [deleteTarget, setDeleteTarget] = useState<CategProveedorOption | null>(null)
  ...
  function openEdit(item: CategProveedorOption) {
    const run = () => {
  ...
  function duplicateItem(item: CategProveedorOption) {
    const run = () => {
      setSelectedId(null)
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const url = form.id ? `${apiUrl("/api/cxp/supplier-categories")}/${form.id}` : apiUrl("/api/cxp/supplier-categories")
        ...
        const result = (await res.json()) as { ok: boolean; message?: string; data?: CategProveedorOption }
  ...
  async function handleDelete(id: number) {
    if (!confirm("¿Eliminar esta categoria de proveedor?")) return
    startTransition(async () => {
      try {
        const res = await fetch(`${apiUrl("/api/cxp/supplier-categories")}/${id}`, { method: "DELETE", credentials: "include" })
  ...
  return (
    <section className="data-panel">
        ...
        <aside className="price-lists-sidebar">
          <div className="price-lists-sidebar__header">
            <div className="price-lists-sidebar__title">
              <Layers size={17} />
              <h2>Categorias de Proveedor</h2>
            </div>
            <button className="sidebar__add-btn" type="button" onClick={openNew} title="Nueva categoria">
              <Plus size={15} />
            </button>
          </div>
          ...
          {selected || isEditing ? (
            <form className="price-lists-form" onSubmit={onSubmit}>
              <div className="price-lists-form__header">
                <h3>{form.id ? form.name : "Nueva Categoria de Proveedor"}</h3>
            ...
            <div className="price-lists-empty">
              ...
              <p>Selecciona una categoria de proveedor o crea una nueva</p>
            </div>
          )}
        </main>
      </div>
      ...
      <DeleteConfirmModal
        ...
        entityLabel="Categoria de Proveedor"
