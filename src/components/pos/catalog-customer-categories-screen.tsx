...
import type { CategClienteOption } from "@/lib/pos-data"
...
function recordToForm(r: CategClienteOption): ItemForm {
  return { id: r.id, code: r.code, name: r.name, active: r.active }
}

function duplicateRecordToForm(r: CategClienteOption): ItemForm {
  return { code: "", name: `${r.name} COPIA`, active: r.active }
}

export function CatalogCustomerCategoriesScreen({ initialData }: { initialData: CategClienteOption[] }) {
  const router = useRouter()
  const { t } = useI18n()
  const { setDirty, confirmAction } = useUnsavedGuard()
  const menuRef = useRef<HTMLUListElement | null>(null)

  const [items, setItems] = useState<CategClienteOption[]>(initialData)
  ...
  const [deleteTarget, setDeleteTarget] = useState<CategClienteOption | null>(null)
  ...
  function openEdit(item: CategClienteOption) {
    const run = () => {
  ...
  function duplicateItem(item: CategClienteOption) {
    const run = () => {
      setSelectedId(null)
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const url = form.id ? `${apiUrl("/api/cxc/customer-categories")}/${form.id}` : apiUrl("/api/cxc/customer-categories")
        ...
        const result = (await res.json()) as { ok: boolean; message?: string; data?: CategClienteOption }
  ...
  async function handleDelete(id: number) {
    if (!confirm("¿Eliminar esta categoria de cliente?")) return
    startTransition(async () => {
      try {
        const res = await fetch(`${apiUrl("/api/cxc/customer-categories")}/${id}`, { method: "DELETE", credentials: "include" })
  ...
  return (
    <section className="data-panel">
          ...
          {selected || isEditing ? (
            <form className="price-lists-form" onSubmit={onSubmit}>
              <div className="price-lists-form__header">
                <h3>{form.id ? form.name : "Nueva Categoria de Cliente"}</h3>
            ...
            <div className="price-lists-empty">
              ...
              <p>Selecciona una categoria de cliente o crea una nueva</p>
            </div>
          )}
        </main>
      </div>
      ...
      <DeleteConfirmModal
        ...
        entityLabel="Categoria de Cliente"
