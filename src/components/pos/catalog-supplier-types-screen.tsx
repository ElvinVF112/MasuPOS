...
import type { TipoProveedorOption } from "@/lib/pos-data"
...
function recordToForm(r: TipoProveedorOption): ItemForm {
  return { id: r.id, code: r.code, name: r.name }
}

function duplicateRecordToForm(r: TipoProveedorOption): ItemForm {
  return { code: "", name: `${r.name} COPIA` }
}

export function CatalogSupplierTypesScreen({ initialData }: { initialData: TipoProveedorOption[] }) {
  const router = useRouter()
  const { t } = useI18n()
  const { setDirty, confirmAction } = useUnsavedGuard()
  const menuRef = useRef<HTMLUListElement | null>(null)

  const [items, setItems] = useState<TipoProveedorOption[]>(initialData)
  ...
  const [deleteTarget, setDeleteTarget] = useState<TipoProveedorOption | null>(null)
  ...
  function openEdit(item: TipoProveedorOption) {
    const run = () => {
  ...
  function duplicateItem(item: TipoProveedorOption) {
    const run = () => {
      setSelectedId(null)
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const url = form.id ? `${apiUrl("/api/cxp/supplier-types")}/${form.id}` : apiUrl("/api/cxp/supplier-types")
        ...
        const result = (await res.json()) as { ok: boolean; message?: string; data?: TipoProveedorOption }
  ...
  async function handleDelete(id: number) {
    if (!confirm("¿Eliminar este tipo de proveedor?")) return
    startTransition(async () => {
      try {
        const res = await fetch(`${apiUrl("/api/cxp/supplier-types")}/${id}`, { method: "DELETE", credentials: "include" })
          ...
          {selected || isEditing ? (
            <form className="price-lists-form" onSubmit={onSubmit}>
              <div className="price-lists-form__header">
                <h3>{form.id ? form.name : "Nuevo Tipo de Proveedor"}</h3>
            ...
            <div className="price-lists-empty">
              ...
              <p>Selecciona un tipo de proveedor o crea uno nuevo</p>
            </div>
          )}
        </main>
      </div>
