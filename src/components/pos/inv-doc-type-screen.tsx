...
import type { InvTipoDocumentoRecord, InvTipoDocUsuarioRecord, InvTipoOperacion } from "@/lib/pos-data"
...
type DocTypeOption = { id: number; description: string; prefijo: string }
...
function recordToForm(r: InvTipoDocumentoRecord): ItemForm {
  return {
    id: r.id,
    ...
    actualizaCosto: r.actualizaCosto,
    idMoneda: r.idMoneda,
    idTipoDocumentoEntrada: r.idTipoDocumentoEntrada,
    idTipoDocumentoSalida: r.idTipoDocumentoSalida,
    active: r.active,
  }
}

function emptyForm(tipoOperacion: InvTipoOperacion): ItemForm {
  return {
    tipoOperacion,
    description: "",
    prefijo: "",
    secuenciaInicial: 1,
    actualizaCosto: tipoOperacion === "C",
    idMoneda: null,
    idTipoDocumentoEntrada: null,
    idTipoDocumentoSalida: null,
    active: true,
  }
}
...
type Props = {
  tipoOperacion: InvTipoOperacion
  title: string
  initialData: InvTipoDocumentoRecord[]
  currencies: CurrencyOption[]
  entryDocTypes?: DocTypeOption[]
  exitDocTypes?: DocTypeOption[]
}

export function InvDocTypeScreen({ tipoOperacion, title, initialData, currencies, entryDocTypes = [], exitDocTypes = [] }: Props) {
  const router = useRouter()
  const { t } = useI18n()
  const { setDirty, confirmAction } = useUnsavedGuard()
  const menuRef = useRef<HTMLUListElement | null>(null)

  const [items, setItems] = useState<InvTipoDocumentoRecord[]>(initialData)
  ...
  const [users, setUsers] = useState<InvTipoDocUsuarioRecord[]>([])
  const [usersLoading, setUsersLoading] = useState(false)
  const [deleteTarget, setDeleteTarget] = useState<InvTipoDocumentoRecord | null>(null)
  ...
  const canToggleCostUpdate = isEditing && (["E", "C"] as InvTipoOperacion[]).includes(tipoOperacion)
  ...
  async function loadUsers(id: number) {
    ...
    try {
      const res = await fetch(apiUrl(`/api/inventory/doc-types/${id}?section=users`), { credentials: "include" })
      const result = (await res.json()) as { ok: boolean; data?: InvTipoDocUsuarioRecord[]; message?: string }
  ...
  function openEdit(item: InvTipoDocumentoRecord) {
    const run = () => {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (form.tipoOperacion === "T" && !form.idTipoDocumentoEntrada) { setMessage("El tipo de documento de entrada es obligatorio."); return }
    if (form.tipoOperacion === "T" && !form.idTipoDocumentoSalida) { setMessage("El tipo de documento de salida es obligatorio."); return }

    startTransition(async () => {
      try {
        const url = form.id
          ? apiUrl(`/api/inventory/doc-types/${form.id}`)
          : apiUrl("/api/inventory/doc-types")
        const res = await fetch(url, {
          method: form.id ? "PUT" : "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ...form, description: (form.description || "").toUpperCase() }),
        })
        const result = (await res.json()) as { ok: boolean; message?: string; data?: InvTipoDocumentoRecord }
  ...
  async function handleDelete(id: number) {
    ...
    startTransition(async () => {
      try {
        const res = await fetch(apiUrl(`/api/inventory/doc-types/${id}`), { method: "DELETE", credentials: "include" })
  ...
  function duplicateItem(item: InvTipoDocumentoRecord) {
    const run = () => {
      setSelectedId(null)
      setForm({
        ...recordToForm(item),
  ...
  async function handleToggleUser(userId: number, currentlyAssigned: boolean) {
    ...
    try {
      const res = await fetch(apiUrl(`/api/inventory/doc-types/${selectedId}`), {
        method: "PATCH",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userIds: newIds }),
      })
      const result = (await res.json()) as { ok: boolean; data?: InvTipoDocUsuarioRecord[] }
      if (result.ok && result.data) {
        setUsers(result.data)
            ...
            {filteredItems.map((item) => (
              ...
              >
                ...
                <p className="price-lists-sidebar__meta">
                  ...
                  {item.simboloMoneda ? ` Â· ${item.simboloMoneda}` : ""}
                </p>
              </div>
            ))}
          </div>
              ...
              {tab === "general" && (
                <div className="price-lists-form">
                  <div className="form-grid">
                    ...
                    <label className="form-grid__toggle">
                      <span>Actualiza Costo</span>
                      <button
                        type="button"
                        className={form.actualizaCosto ? "toggle-switch is-on" : "toggle-switch"}
                        onClick={() => {
                          if (!canToggleCostUpdate) return
                          setForm((prev) => ({ ...prev, actualizaCosto: !prev.actualizaCosto }))
                        }}
                        disabled={!canToggleCostUpdate}
                      >
                        <span />
                      </button>
                    </label>

                    {form.tipoOperacion === "T" ? (
                      <>
                        <label>
                          <span>Tipo Doc. Entrada *</span>
                          <select
                            value={form.idTipoDocumentoEntrada ?? ""}
                            onChange={(e) => setForm({ ...form, idTipoDocumentoEntrada: e.target.value ? Number(e.target.value) : null })}
                            disabled={!isEditing}
                          >
                            <option value="">Seleccionar...</option>
                            {entryDocTypes.map((item) => (
                              <option key={item.id} value={item.id}>
                                {item.description}{item.prefijo ? ` (${item.prefijo})` : ""}
                              </option>
                            ))}
                          </select>
                        </label>
                        ...
                        <label>
                          <span>Tipo Doc. Salida *</span>
                          <select
                            value={form.idTipoDocumentoSalida ?? ""}
                            onChange={(e) => setForm({ ...form, idTipoDocumentoSalida: e.target.value ? Number(e.target.value) : null })}
                            disabled={!isEditing}
                          >
                            <option value="">Seleccionar...</option>
                            {exitDocTypes.map((item) => (
                              <option key={item.id} value={item.id}>
                                {item.description}{item.prefijo ? ` (${item.prefijo})` : ""}
                              </option>
                            ))}
