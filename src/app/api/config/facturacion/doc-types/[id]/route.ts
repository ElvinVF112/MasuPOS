...
import { saveFacTipoDocumento, deleteFacTipoDocumento, getFacTipoDocUsuarios, syncFacTipoDocUsuarios } from "@/lib/pos-data"
...
export async function GET(request: Request, { params }: Context) {
  ...
  try {
    ...
    if (searchParams.get("section") === "users") {
      const data = await getFacTipoDocUsuarios(Number(id))
...
export async function PUT(request: Request, { params }: Context) {
  ...
  try {
    ...
    const result = await saveFacTipoDocumento({ ...body, id: Number(id) })
...
export async function DELETE(request: Request, { params }: Context) {
  ...
  try {
    const { id } = await params
    await deleteFacTipoDocumento(Number(id))
...
export async function PATCH(request: Request, { params }: Context) {
  ...
  try {
    ...
    const data = await syncFacTipoDocUsuarios(Number(id), body.userIds as number[])
    return NextResponse.json({ ok: true, data })
  } catch (error) {
