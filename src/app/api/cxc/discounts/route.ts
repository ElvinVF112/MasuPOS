...
import { getDescuentos, saveDescuento } from "@/lib/pos-data"

export async function GET(request: Request) {
  const auth = await requireApiSession(request)
  if (!auth.ok) return auth.response
  try {
    return NextResponse.json({ ok: true, data: await getDescuentos() })
  } catch (error) {
    return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "Error al cargar descuentos." }, { status: 400 })
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "No se pudo crear el descuento." }, { status: 400 })
  }
}
