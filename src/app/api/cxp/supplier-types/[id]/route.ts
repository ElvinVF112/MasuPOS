...
import { saveTipoProveedor, deleteTipoProveedor } from "@/lib/pos-data"
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await saveTipoProveedor({ id: numId, code: body.code, name: body.name })
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const numId = Number(id)
    if (isNaN(numId) || numId <= 0) return NextResponse.json({ ok: false, message: "ID invalido." }, { status: 400 })
    await deleteTipoProveedor(numId)
