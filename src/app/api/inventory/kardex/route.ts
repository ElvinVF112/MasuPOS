...
import { getInvKardex } from "@/lib/pos-data"
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await getInvKardex(idProducto, idAlmacen, desde, hasta)
    return NextResponse.json({ ok: true, data })
  } catch (error) {
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "No se pudo consultar kardex." },
      { status: 400 },
    )
  }
}
