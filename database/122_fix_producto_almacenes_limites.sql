...
BEGIN
  CREATE TABLE dbo.ProductoAlmacenesLimites (
    ...
    Minimo DECIMAL(18,4) NOT NULL CONSTRAINT DF_ProductoAlmacenesLimites_Minimo DEFAULT(0),
    Maximo DECIMAL(18,4) NOT NULL CONSTRAINT DF_ProductoAlmacenesLimites_Maximo DEFAULT(0),
    PuntoReorden DECIMAL(18,4) NOT NULL CONSTRAINT DF_ProductoAlmacenesLimites_PuntoReorden DEFAULT(0),
    RowStatus BIT NOT NULL CONSTRAINT DF_ProductoAlmacenesLimites_RowStatus DEFAULT(1),
    FechaCreacion DATETIME NOT NULL CONSTRAINT DF_ProductoAlmacenesLimites_FechaCreacion DEFAULT(GETDATE()),
    UsuarioCreacion INT NULL,
    FechaModificacion DATETIME NULL,
    UsuarioModificacion INT NULL,
    CONSTRAINT UQ_ProductoAlmacenesLimites UNIQUE (IdProducto, IdAlmacen)
  );
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_PAL_Producto' AND parent_object_id = OBJECT_ID('dbo.ProductoAlmacenesLimites'))
BEGIN
  ALTER TABLE dbo.ProductoAlmacenesLimites
    ADD CONSTRAINT FK_PAL_Producto FOREIGN KEY (IdProducto) REFERENCES dbo.Productos(IdProducto);
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_PAL_Almacen' AND parent_object_id = OBJECT_ID('dbo.ProductoAlmacenesLimites'))
BEGIN
  ALTER TABLE dbo.ProductoAlmacenesLimites
    ADD CONSTRAINT FK_PAL_Almacen FOREIGN KEY (IdAlmacen) REFERENCES dbo.Almacenes(IdAlmacen);
END
GO
...
BEGIN
  ...
  BEGIN
    ...
    LEFT JOIN dbo.ProductoAlmacenesLimites pal ON pal.IdProducto = pa.IdProducto
  ...
  IF @Accion = 'U'
  BEGIN
    IF EXISTS (SELECT 1 FROM dbo.ProductoAlmacenesLimites WHERE IdProducto = @IdProducto AND IdAlmacen = @IdAlmacen)
    BEGIN
    ...
    ELSE
    BEGIN
      INSERT INTO dbo.ProductoAlmacenesLimites (IdProducto, IdAlmacen, Minimo, Maximo, PuntoReorden, RowStatus, FechaCreacion, UsuarioCreacion)
      VALUES (@IdProducto, @IdAlmacen, ISNULL(@Minimo, 0), ISNULL(@Maximo, 0), ISNULL(@PuntoReorden, 0), 1, GETDATE(), @IdSesion);
    END

    SELECT TOP 1 IdProducto, IdAlmacen, Minimo, Maximo, PuntoReorden
...
BEGIN
    ...
    SELECT al.IdAlmacen,
           ...
           uc.BaseA AS BaseACompra,
           uc.BaseB AS BaseBCompra,
           CASE
               WHEN p.IdUnidadCompra IS NULL THEN NULL
               WHEN ISNULL(uc.BaseA, 0) <= 0 OR ISNULL(uc.BaseB, 0) <= 0 THEN NULL
               ELSE CAST((ISNULL(pa.Cantidad, 0) - ISNULL(pa.CantidadReservada, 0)) / (CAST(uc.BaseB AS decimal(18, 6)) / NULLIF(CAST(uc.BaseA AS decimal(18, 6)), 0)) AS decimal(18, 4))
           END AS DisponibleUnitCompra,
           -- UNIDAD DE VENTA
           p.IdUnidadVenta,
