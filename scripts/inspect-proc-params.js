...
async function run() {
  ...
  const targets = ["spRolesCRUD", "spPermisosCRUD", "spMesasCRUD", "spCategoriasCRUD", "spAreasCRUD", "spRecursosCRUD"]
  const pool = await new sql.ConnectionPool(cs).connect()
  try {
    const req = pool.request()
    targets.forEach((name, i) => req.input(`p${i}`, name))
    const inList = targets.map((_, i) => `@p${i}`).join(",")
    const query = `
      SELECT
        P.name AS ProcName,
        PR.parameter_id AS ParamOrder,
